...
def _placeAlgoOrder(
    orderType: str, symbol: str, side: str, quantity: str,
    triggerPrice: str, baseUrl: str, apiKey: str, apiSecret: str,
) -> int:
    """POST /fapi/v1/algoOrder. Return algoId. Shared by placeStopLoss + placeTakeProfit."""
    params: dict[str, str | int] = {
        "symbol": symbol, "side": side, "quantity": quantity,
        "algoType": "CONDITIONAL", "type": orderType,
    ...
    return int(response["algoId"])


def placeStopLoss(
    symbol: str, side: str, quantity: str, triggerPrice: str,
    baseUrl: str, apiKey: str, apiSecret: str,
) -> int:
    """
    POST /fapi/v1/algoOrder type=STOP_MARKET.
    workingType=MARK_PRICE — prevent candle spike trigger.
    Return: algoId (int) — BUKAN orderId.
    """
    algoId = _placeAlgoOrder(
        "STOP_MARKET", symbol, side, quantity, triggerPrice, baseUrl, apiKey, apiSecret
    )
...
def placeTakeProfit(
    ...
    """
    POST /fapi/v1/algoOrder type=TAKE_PROFIT_MARKET.
    Return: algoId (int).
    """
    algoId = _placeAlgoOrder(
        "TAKE_PROFIT_MARKET", symbol, side, quantity, triggerPrice, baseUrl, apiKey, apiSecret
    )
...
def cancelAlgoOrder(
    symbol: str, algoId: int, baseUrl: str, apiKey: str, apiSecret: str,
) -> None:
    """
    DELETE /fapi/v1/algoOrder — pakai algoId, BUKAN orderId.
...
def listOpenAlgoOrders(
    ...
    if isinstance(response, list):
        orders: list[dict[str, str | int | float]] = response
    else:
        orders = []
